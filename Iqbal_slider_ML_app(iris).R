...
ui <- pageWithSidebar(
  ...
  # Page header
  headerPanel('Species Predictor Iris data'),
  
  # Input values
  sidebarPanel(
    HTML("<h3>Input parameter values</h4>"),
    sliderInput("Sepal.Length", label = "Sepal Length", value = 5.0,
                min = min(TrainSet$Sepal.Length),
                max = max(TrainSet$Sepal.Length)
    ),
    sliderInput("Sepal.Width", label = "Sepal Width", value = 3.5,
                min = min(TrainSet$Sepal.Width),
                max = max(TrainSet$Sepal.Width)),
    sliderInput("Petal.Length", label = "Petal Length", value = 1.5,
